...
public static class McpToolHelpCatalog
{
    ...
    private static IReadOnlyDictionary<string, McpToolHelpEntry> Build() =>
        new Dictionary<string, McpToolHelpEntry>(StringComparer.Ordinal)
        {
            ...
            {
                Workflow = "Use after publish/reload to confirm the binary path, profile, startup groups, dynamic groups, and current tool count. Also shows RoslynMcp.jsonc paths read (exe then cwd), merged keys, unknown-key warnings, parse failures, and whether the solution came from the file or load_workspace. Does not start a lazy load.",
            },
            ["load_workspace"] = new()
            {
                Prerequisites = "workspacePath must be a .sln, .slnx, or .csproj file, not a directory.",
                Workflow = "Call this before C# analysis unless RoslynMcp.jsonc already set workspace-path (first semantic tool loads that path and waits; it does not cancel an in-progress load). Compares only arguments you passed: omitted configuration/platform/targetFramework/buildArgs do not clear already-loaded values. Passing a different configuration reloads with that configuration and keeps platform/TFM you did not mention. Prefer a solution file for multi-config repos. Pass targetFramework when the project uses TargetFrameworks. run_dotnet_build and run_dotnet_test inherit configuration/platform when omitted. Optional buildArgs is a session suffix for later `dotnet build` (probe and pre-test build). On a large .sln pass briefOutput=true to collapse MSBuild/NuGet warnings; failures still print in full. logProjectOutputDiagnostics=true logs per-project OutputFilePath/GeneratedFilesOutputDirectory and AnalyzerReference existence/timestamp to the MCP log — use when an analyzer/generator project is not producing generated sources under a Directory.Build.props that overrides OutputPath. shadowCopyInSolutionAnalyzers=true fixes that case: rewrites a provenance-confirmed AnalyzerReference to a shadow copy of the referenced project's own resolved output (requires that project already built once). Activation is session-sticky: cached false/omitted preserves an active overlay; reset_workspace or another load key clears it. Not a RoslynMcp.jsonc key.",
                Pitfalls = "Host abort mid-load is a client timeout, not an MSBuild failure. Restore/design-time warnings do not fail load; NU/MSB/NETSDK errors do. A changed .csproj/.sln does not auto-reopen MSBuild — call again or reset_workspace. Unsaved editor buffers are ignored. logProjectOutputDiagnostics output goes only to the server log (tail_tool_log / read_log_tail), not to this tool's return value. shadowCopyInSolutionAnalyzers requires complete load-session provenance; filename similarity is not evidence, and the tool does not build analyzers itself. Without ROSLYN_MCP_WORKSPACE, cwd is often the user profile so RoslynMcp.jsonc may be read from the wrong place.",
                RelatedTools = ["reset_workspace", "run_dotnet_build", "run_dotnet_test", "get_mcp_server_info"],
            },
            ["reset_workspace"] = new()
            {
                Workflow = "Use after building so the next load_workspace picks up generated obj files and project-graph changes. Also clears a lazy RoslynMcp.jsonc load; the next semantic tool may load again from the file if workspace-path is set.",
