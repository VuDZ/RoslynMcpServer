# Mapping: `roslyn_mcp_tool_review.md` — proposal v1 (superseded)
...
| R-§2 | Слить `find_symbol_references` и `find_usages` в `find_references` | ACCEPT WITH MODIFICATION | Stage 2: optional location/`symbolId` на `find_usages`. Имя `find_symbol_references` не удалять в 1.x. Rename → Stage 3 |
...
| R-§5 | `get_symbol_info` + `symbolId`; доп. поля (generics, XML, origin) | ACCEPT WITH MODIFICATION | Stage 2: минимальный JSON + `symbolId`. Расширенные поля — не блокер первой поставки |
| R-§6 | Symbol-oriented API / `resolve_symbol` → handle | ACCEPT WITH MODIFICATION | Stage 2: Roslyn `SymbolKey` как opaque string; отдельный `resolve_symbol` не обязателен, если `get_symbol_info` резолвит query/location/`symbolId`. Чистое имя `resolve_symbol` — Stage 3 |
...
| R-§22 | Стабильные id ещё для diagnostic/fix/test | DEFER | Stage 3; 1.x только `symbolId` |
...
| R-§26 | Product direction: reliability, tokens, safe edits, identity, measurable outcomes | ACCEPT | рамка серии; метрики (§26.6) не обещать в 1.x без измерения |
