...
| R-§3 | Skeleton + method body — центральная идея; обобщить до `get_symbol_source` / `get_symbol_outline` | ACCEPT | Stage 1: body в `core`. Stage 2: `get_symbol_source`; ID-режим `get_method_body` = snapshot. Outline: `get_class_skeleton` в 1.x; generic `get_symbol_outline(symbolId)` — Stage 3 |
