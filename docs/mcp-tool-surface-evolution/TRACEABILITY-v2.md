...
| E2-02 | AWM | ID-only для пяти старых tools; XOR режимы; `newName` required | Stage 2 «Публичные формы вызовов» + Descriptions |
| E2-03 | AWM | Project context в ID; location candidates; legacy first-hit оговорён | Stage 2 location; U-ARB-03 |
| E2-04 | AWM | Любая правка документа объявления → `stale-id`; другой файл не инвалидирует | Stage 2 lifetime / проверка / payload `stale-id` / тесты |
...
| Прозрачный re-issue ID по якорю после stale-id | post-v2 UX | Stage 2 проверка / «Не входит»; payload last-known |
| Инкрементальный content hash | post-v2 perf | Stage 2 store: SourceText checksum only |
| `get_symbol_outline` в Stage 2 | R-§3 развилка | Stage 3 вопрос 3 |
...
## 5. Incoming review mapping

Вердикты M-* и R-§* сохранены. Нормативные правки mapping: R-§6 не
предписывает `SymbolKey`; R-§3 generic outline — Stage 3. См.
