...
[`roslyn_mcp_tool_review.md`](_archive/roslyn_mcp_tool_review.md). Ниже — что
спека **обязана** решить, не копируя review как истину.

Арбитраж 1.x не превращает этот файл в JSON-контракт. Stage 3 **не**
наследует непубличный `SymbolKey` и **не** считает Stage 2 гарантией
переносимого ID.
...
6. **Stable ids сверх символов:** `diagnosticId`, `codeFixId`, `testId`
   ...
   filter needles. Если понадобится ID, переживающий процесс или правку
   документа объявления — это отдельный механизм, не lifetime Stage 2.
...
(или выделенный `docs/roslyn-mcp-v2/` после собственного arbitration) и
запись в [`docs/README.md`](../README.md). Пока этого цикла нет — этот
файл остаётся напоминанием границ, не JSON-контрактом.
