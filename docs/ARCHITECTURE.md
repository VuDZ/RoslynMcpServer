...
- [`docs/archive/compact-tools/`](archive/compact-tools/README.md) records the historical implementation epochs for the compact catalog; it is not the operator or contributor architecture reference. See [`docs/README.md`](README.md) for the docs index.
- [`docs/workspace-load-cache/`](workspace-load-cache/README.md) is a **forward-looking** post-arbitration plan (faster load of large solutions, disk evaluation snapshot). It is not shipped behavior. Do not treat those epochs as the current workspace lifecycle.
...
- The fix for the case above is `load_workspace`'s optional `shadowCopyInSolutionAnalyzers` flag (`Services/AnalyzerReferenceShadowCopier.cs`): for every `AnalyzerReference` whose complete load-session provenance exact-joins it to one loaded in-solution project (filename/`AssemblyName` similarity is not evidence), it copies that project's own resolved output (`CompilationOutputInfo.AssemblyPath` / `OutputFilePath`) into an immutable content-hashed generation under `v2-main-only/` in a private per-solution shadow root (OS temp). v1.3.5 used last-write-time directory names and recopied on document edit; v1.3.6 prepares at load/enable/explicit refresh only and reapplies an in-memory original↔shadow mapping with no analyzer file I/O. Loading happens through `Services/InProcessAnalyzerAssemblyLoader.cs`, a process-lifetime public-API-only `IAnalyzerAssemblyLoader` owned by `SolutionManager`. v1.3.7 (epoch 3) chose **restart-required** for same-identity generator updates: cached load and `reset_workspace` prepare new files but refuse in-process execution with an explicit restart action; a new MCP process runs V2. Supported dependency policy is **main-only** — private helpers are refused, not copied from output and not resolved by first-match simple name. `reset_workspace` does not unload CLR assemblies. This also solves the second half of the pitfall: the MCP process only ever locks the shadow copy, never the analyzer project's real build output, so a later `dotnet build` of that project does not fail with `MSB3027`. Removal condition: drop this workaround if Roslyn ships a public non-locking analyzer loader, or if MSBuildWorkspace's own `AnalyzerReference` resolution is fixed to honor a referenced project's actual `OutputPath`.
  ...
  - Full history of this investigation — MVP repro, v1.3.4 disk corruption, v1.3.5 in-memory overlay, v1.3.6 immutable mapping, v1.3.8 write boundary — is in [`docs/archive/analyzer-shadow-copy/`](archive/analyzer-shadow-copy/README.md) and [`docs/archive/analyzer-shadow-copy-improvements/v2/`](archive/analyzer-shadow-copy-improvements/v2/README.md). Read it before changing this area again.
- `shadowCopyInSolutionAnalyzers` is opt-in and is never inferred by the server or client. When enabled, load/refresh prepares content-hashed generations under the OS temp directory. `AnalyzerShadowMapping.Apply` is a pure `Solution` transform over a ready mapping — it is not the copier. Document synchronization reapplies that mapping without analyzer I/O. Activation is session-sticky: cached `false`/omitted does not disable or refresh an already-active overlay; turn it off with `reset_workspace` then load without the flag (another load key/graph reopen also starts without the old overlay). After a generator rebuild with the same assembly identity, restart the MCP process — `reset_workspace` does not unload CLR assemblies. A missing, Failed, Incomplete, or other-session provenance capture withholds the semantic snapshot (`Unavailable`, `_solution` null); cached `true` does not repair it — recovery is `reset_workspace` then a new load (CLR is still not unloaded). After a failed opt-in, raw workspace may keep original analyzer refs for persistence; that is not permission to publish or execute them. Admission (NoOverlay / AllowedMapping / Banned) survives edit, flush, reconciliation, cancel, and cached `false`. Mixed prepare decides every confirmed overlay ref; a failed confirmed ref is never left on the real path because another prepare succeeded. An inaccessible original `AnalyzerReference` path is **skip** (U-ARB-01, v1.3.21): `access_failure`, not missing, not rewritten from a confirmed project output, and not a whole-snapshot fail-closed. The load summary says so. Rewrite or fail-closed remain documented alternatives and may replace skip after real-repo evidence — see [u-arb-01-inaccessible-skip.md](archive/analyzer-shadow-copy-improvements/v2/u-arb-01-inaccessible-skip.md). Source-output access failure is a separate prepare failure. Layout namespace is `v2-main-only`; a future dependency-set policy would use a different namespace. Timestamp directories from v1.3.5 are not migrated in place. Published generations have no automatic GC and are not deleted on workspace clear; operator cleanup requires every owning process to have stopped and the path to sit under the known cache parent. Historical v1.3.4 `.csproj` corruption is not auto-repaired. See [v3 S3](archive/analyzer-shadow-copy-improvements/v3/s3-acceptance.md), [S4](archive/analyzer-shadow-copy-improvements/v3/s4-acceptance.md), [S5](archive/analyzer-shadow-copy-improvements/v3/s5-acceptance.md).
- U-ARB-04 is shipped in v1.3.14: `LoadWorkspace` routes physical load/cache lookup, optional prepare, execution gate, and final publication through one `SolutionManager.LoadAndPrepareAsync` acquisition. Production semantic readers use serialized published-snapshot accessors (or `FindDocumentAsync`, which uses the same lock) and never fall back to raw `workspace.CurrentSolution`. `GetCurrentSolution()` returns only the published `_solution`. A deterministic seam between physical load and prepare proves that a semantic request waits and then compiles the shadow path; new-session prepare failure/cancellation is fail-closed and stays fail-closed across later mutation (v3 S3). A no-overlay semantic operation completed before later enablement remains subject to the restart-required identity gate. See the [implementation acceptance](archive/analyzer-shadow-copy-improvements/v2/u-arb-04-implementation-acceptance.md).
- CodeAction / semantic rename persist through the same write boundary. An unsupported or stale analyzer-reference diff is rejected **before** any server write (exact inverse of known overlay replacements only — not a silent whole-list wipe). If only some files are saved, the tool reply is not full success: it reports Status, Reason, and the known saved paths.
- Path resolution (`ResolvePathAgainstWorkspace`) and the anti-lock shadow-copy workaround are independent. Fixing one does not remove the other. `find_symbol_definition` by generated type name is a Roslyn declaration-search limitation, not evidence that generation or the semantic model failed.
...
AST tools, code fixes, and semantic rename build a candidate `Solution` with Roslyn syntax APIs, `DocumentEditor`, formatting, and `CodeAction`. Persist goes through `SolutionManager.ApplySolutionChangesToDiskAsync` (the v1.3.8 write boundary): preflight, exact inverse of known overlay analyzer replacements, then disk write and `TryApplyChanges` only on the cleaned candidate — never the overlay `Solution`. Production `TryApplyChanges` exists only inside that boundary. Unsupported or stale analyzer diffs are rejected before any server write; partial persistence reports Status, Reason, and known saved paths. Text replacement and full-file writes exist for non-semantic or headless workflows, but an IDE-native editor remains preferable when its diff review is part of the user workflow.
...
- `run_dotnet_build` reports coarse MCP `notifications/progress` while a step runs (step label, that step's elapsed — `starting` on a boundary — and the previous exit; no stdout, and the numeric field is elapsed seconds rather than a completion percentage) through the `DotNetCliRunner`/`DotNetBuildProbe` seam. `run_dotnet_test` / `run_specific_test` / `run_test_by_filter` / `run_dotnet_run` use the same reporter: labels `dotnet build` / `dotnet test` / `dotnet run`, 5 s heartbeat, no VSTest/stdout lines. Pre-test compile is not the build probe. It is a heartbeat for a live agent, not a result channel and not a way to extend a host `tools/call` timeout.
