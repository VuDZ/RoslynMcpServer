...
- U-ARB-04 evidence bounds the anti-lock statement: on Windows, physical load/prepare is lazy, and the supported overlay semantic/write paths load the shadow assembly and allow forced replacement of an existing-correct real output. A test-only semantic compilation from raw `workspace.CurrentSolution` before enable loads and locks the real DLL (`MSB3021`), then prevents overlay activation through the identity gate. No explicit production raw semantic reader was found; concurrent dispatch in the interval between physical load and prepare was not reproduced. This evidence does not define a new load boundary.
