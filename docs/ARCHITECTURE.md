...
- `run_dotnet_build` reports coarse MCP `notifications/progress` while a step runs (step label, that step's elapsed — `starting` on a boundary — and the previous exit; no stdout, and the numeric field is elapsed seconds rather than a completion percentage) through the `DotNetCliRunner`/`DotNetBuildProbe` seam. It is a heartbeat for a live agent, not a result channel and not a way to extend a host `tools/call` timeout; `run_dotnet_test` / `run_specific_test` / `run_dotnet_run` do not report progress yet.
