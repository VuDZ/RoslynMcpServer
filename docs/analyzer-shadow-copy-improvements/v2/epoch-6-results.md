...
| Серия завершена | **нет** — эпоха 5 не rollout (U-ARB-01); U-ARB-04/05 открыты |

Эпоха 3 документируется как **выбранный restart-required / main-only**, не как
исследовательский failure и не как in-process V2. Эпоха 5 — **deferred**, не
implemented.
...
- Эпоха 5 / U-ARB-01: выпущенный filename matcher, rollout запрещён.
