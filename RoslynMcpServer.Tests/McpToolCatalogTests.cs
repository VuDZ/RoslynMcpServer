...
public sealed class McpToolCatalogTests(ITestOutputHelper output)
{
    ...
    {
        AssertRecorded("full", 62, 39408, MeasureSurface(new McpToolProfileOptions { Profile = "full" }));
        AssertRecorded("lite", 18, 12173, MeasureSurface(new McpToolProfileOptions { Profile = "lite" }));
        AssertRecorded("lite+files", 25, 16401, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "files" }));
        AssertRecorded("lite+editing", 35, 23049, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "editing" }));
        AssertRecorded("lite+decompile", 22, 15091, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "decompile" }));
        AssertRecorded("lite+nuget", 24, 15786, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "nuget" }));
        AssertRecorded("lite+project", 21, 13810, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "project" }));
        AssertRecorded("lite+runtime", 21, 14219, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "runtime" }));
        AssertRecorded("lite+operations", 22, 14090, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "operations" }));

        var enabled = MeasureSurface(
            new McpToolProfileOptions { Profile = "lite" },
            activation => activation.EnableGroup("files"));
        AssertRecorded("lite+enable:files", 25, 16401, enabled);
        Assert.Equal(16401, MeasureSurface(new McpToolProfileOptions { Profile = "lite", Groups = "files" }).Utf8Bytes);
    }
